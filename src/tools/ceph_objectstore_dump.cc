...
void usage(po::options_description &desc)
{
    ...
    cerr << "ceph_objectstore_dump ... <object> (get|set)-bytes [file]" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> (set-(attr|omap) <key> [file]" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> (set-omaphdr) [file]" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> (get|rm)-(attr|omap) <key>" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> (get-omaphdr)" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> list-attrs" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> list-omap" << std::endl;
    cerr << "ceph_objectstore_dump ... <object> remove" << std::endl;
    cerr << std::endl;
    exit(1);
}
...
// The following code for export, info, log require omap or !skip-mount-omap
  if (it != ls.end()) {
    ...
    if (op == "export") {
      ...
      cerr << "Must provide --op (info, log, remove, export, import, list)"
	<< std::endl;
      usage(desc);
    }
  } else {
