...
def main():
    ...
    CFSD_PREFIX = "./ceph_objectstore_dump --data-path dev/{osd} --journal-path dev/{osd}.journal "
    DATALINECOUNT = 10000
    ...
    cmd = "./ceph_objectstore_dump --journal-path dev/{osd}.journal --type memstore --op list --pgid {pg}".format(osd=ONEOSD, pg=ONEPG)
    ERRORS += test_failure(cmd, "Must provide --data-path")

    # Don't specify a journal-path for filestore
    cmd = "./ceph_objectstore_dump --type filestore --data-path dev/{osd} --op list --pgid {pg}".format(osd=ONEOSD, pg=ONEPG)
